<template>
    <div class="flex flex-col gap-4">
        ...
        <div class="rounded-2xl border-[2px] border-dark bg-[#fffaf0] p-2">
            <div class="grid grid-cols-2 gap-2">
                <button
                    ...
                    class="rounded-xl border-[2px] px-4 py-3 text-sm font-black uppercase transition-all"
                    :class="
                        active === 'income'
        ...
        <div class="py-4 max-h-[320px] overflow-y-auto">
            <span
                class="line-clamp-2 text-[12px] font-black uppercase leading-tight"
            >
                Sering Diguakan
            </span>
            ...
            <div v-else class="mt-2 grid grid-cols-4 gap-3">
                <USkeleton
                    v-for="i in 12"
                    :key="i"
                    class="h-[92px] rounded-2xl bg-neutral-300"
                />
            </div>
        </div>
    </div>
...
defineProps({
    selectedCategory: { type: Object as () => iCategory, default: () => ({}) },
});
